...
namespace Quiz {
    public class Solution {
        public bool JudgePoint24(int[] nums) {
            for (var a_i = 0; a_i < nums.Length; a_i++) {
                for (var b_i = 0; b_i < nums.Length; b_i++) {
                    if (b_i == a_i) continue;
                    for (var c_i = 0; c_i < nums.Length; c_i++) {
                        if (c_i == a_i || c_i == b_i) continue;
                        for (var d_i = 0; d_i < nums.Length; d_i++) {
        ...
        private bool CheckOps(double a, double b, double c, double d) {
            for (var br_i = 0; br_i < BRACKETS.Length; br_i++) {
                var br = BRACKETS[br_i];
                for (var op1_i = 0; op1_i < OPS.Length; op1_i++) {
                    var op1 = OPS[op1_i];
                    for (var op2_i = 0; op2_i < OPS.Length; op2_i++) {
                        var op2 = OPS[op2_i];
                        for (var op3_i = 0; op3_i < OPS.Length; op3_i++) {
                            var op3 = OPS[op3_i];
